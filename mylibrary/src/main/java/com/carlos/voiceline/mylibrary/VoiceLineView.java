...
public class VoiceLineView extends View {
    ...
    private boolean leftToRightFlag = false;
    ...
    public void l2r(boolean isLeftToRight){
        leftToRightFlag = isLeftToRight;
        speedY = -speedY;
    }
    ...
    private void drawVoiceRect(Canvas canvas) {
        ...
        if (Math.abs(speedY) % totalWidth < 6) {
            int left = 0;
            int right = 0;
            if(leftToRightFlag){
                 left = (int) (getWidth() - rectWidth + 10 - speedY + (-speedY) % totalWidth);
                 right = (int) (getWidth() + 10 - speedY + (-speedY) % totalWidth);
            }else{
                left = (int) (-rectWidth - 10 - speedY + speedY % totalWidth);
                right = (int) (-10 - speedY + speedY % totalWidth);
            }

            Log.i("TAG", "drawVoiceRect: getWidth()="+getWidth()+";left="+left+";right="+right);
    ...
    public void refreshRect(boolean flag){
        if(!flag) return;
